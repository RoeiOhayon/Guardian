...
dead = Guard(name="dead", predicate=lambda v: v.HP <= 0 and v.velocity == 0, "Don't Perform if dead")
alive = Guard(name="alive", predicate=lambda v: lambda v: v.HP > 0 or v.velocity != 0, description="Don't Perform if alive")
```
main.py:
```python
import custom_guard
